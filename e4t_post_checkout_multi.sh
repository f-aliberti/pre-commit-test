...
if [ $PRE_COMMIT_CHECKOUT_TYPE != '1' ]; then
    echo 'git checkout did not checkout a branch - quitting'
    exit
fi
...
#echo $branch
[[ $branch =~ ^E4T[0-9]{4}$ ]] || echo 'Rename branch with E4TXXXX (X are [0-9] digit)'
...
if [[ $current_version != $new_version ]]; then
    if mvn versions:set-property -Dproperty=revision -DnewVersion=$new_version; then
    ...
    echo "Nothing to change via maven"
fi
